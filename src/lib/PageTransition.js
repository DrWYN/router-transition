...
export default class PageTransition extends React.Component {
  ...
  transite(nextChild, hashAction = 'PUSH') {
    // PUSH or POP
    // const hashAction = 'PUSH';
    // const hashAction = 'POP';
    const isAppear = hashAction === 'PUSH';
    return new Promise((transiteDone, transiteFailed) => {
      ...
      this.forceUpdate(() => {
        ...
        const willStart = () => {
          ...
          } else if (prevChild && prevChild.onTransitionLeaveWillStart) {
            return (
        ...
        const start = () => {
          if (isAppear && newChildDom.classList.contains('transition-item')) {
        ...
        // After add classes
        const didStart = () => {
          if (isAppear && newChild.onTransitionDidStart) {
            return (
              newChild.onTransitionDidStart(this.props.data) ||
              Promise.resolve()
            );
          } else if (prevChild && prevChild.onTransitionLeaveDidStart) {
            return (
        ...
        const willEnd = () => {
          if (isAppear && newChild.onTransitionWillEnd) {
            return (
              newChild.onTransitionWillEnd(this.props.data) || Promise.resolve()
            );
          } else if (prevChild && prevChild.onTransitionLeaveWillEnd) {
            return (
        ...
        const didEnd = () => {
          if (isAppear && newChild.onTransitionDidEnd) {
            return (
              newChild.onTransitionDidEnd(this.props.data) || Promise.resolve()
            );
          } else if (prevChild && prevChild.onTransitionLeaveDidEnd) {
            return (
